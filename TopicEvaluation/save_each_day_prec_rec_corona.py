...
class Evaluate:
    def __init__(self, truth_dir, predicted_dir, top_n_topics, top_n_keywords, assigning_keyword_threshold):
        ...
        # we use these to keep the prev day's cumulative values to extract it from the today's cumulative to find
        # today's exact value:
        self.no_truth_topics2 = 0  # 'no_' means 'number of'
        self.no_found_truth_topics2 = 0
        # for keyword recall
        self.no_truth_keyword2 = 0
        self.no_found_truth_keywords2 = 0
        # for keyword precision
        self.no_prediction_keyword2 = 0
        self.no_found_prediction_keywords2 = 0
    ...
    def get_truth_topics(self):
        ...
        for day_file_name in os.listdir(self.truth_dir):
            ...
            if os.stat(day_topics_dir).st_size == 0:
                ...
                continue
            self.days.append(day_file_name)
    ...
    def assign(self):
        ...
        for ind_day in range(len(self.truth_topics)):
            ################################
            # these will keep the cumulative of the prev day so we can do cum_today - cum_last_day to have exactly today
            self.no_truth_topics2 = self.no_truth_topics
            self.no_found_truth_topics2 = self.no_found_truth_topics
            # for keyword recall
            self.no_truth_keyword2 = self.no_truth_keyword
            self.no_found_truth_keywords2 = self.no_found_truth_keywords
            # for keyword precision
            self.no_prediction_keyword2 = self.no_prediction_keyword
            self.no_found_prediction_keywords2 = self.no_found_prediction_keywords
            ################################
            ...
            # for extracting prec/rec per day:
            print(self.days[ind_day])
            if self.no_found_truth_topics - self.no_found_truth_topics2 == 0:  # if didn't find topic then put 0's
                self.save_in_file('C:/Users/shila/Desktop/covid-stream/Data/outputs/prec-rec-per-day',
                                  self.days[ind_day],
                                  '0.0 0.0 0.0 0.0')
                continue
            save_text = ''
            save_text += str((self.no_found_truth_topics - self.no_found_truth_topics2) / (
                    self.no_truth_topics - self.no_truth_topics2)) + ' '

            save_text += str((self.no_found_prediction_keywords - self.no_found_prediction_keywords2) / (
                    self.no_prediction_keyword - self.no_prediction_keyword2)) + ' '
            save_text += str((self.no_found_truth_keywords - self.no_found_truth_keywords2) / (
                    self.no_truth_keyword - self.no_truth_keyword2)) + ' '
            kr = (self.no_found_truth_keywords - self.no_found_truth_keywords2) / (
                    self.no_truth_keyword - self.no_truth_keyword2)
            kp = (self.no_found_prediction_keywords - self.no_found_prediction_keywords2) / (
                    self.no_prediction_keyword - self.no_prediction_keyword2)
            save_text += str((2 * kr * kp) / (kp + kr))
            self.save_in_file('C:/Users/shila/Desktop/covid-stream/Data/outputs/prec-rec-per-day', self.days[ind_day],
                              save_text)
